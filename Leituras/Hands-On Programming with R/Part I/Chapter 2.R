...
# E para ver a distribuição de apenas uma dessas variáveis? Podemos usar o histograma
## Mas, para isso, criaremos um novo vetor no qual valores se repitam
z <- c(1, 2, 3 ,2 ,2, 4, 4, 5, 6, 2)
qplot(z, binwidth = 1)
qplot(z, bins = 5)
# Nota-se que são duas as opções para definir a largura de cada barra. A primeira é mais prática,
## especialmente porque utilizamos bases com muitas observações e teremos maior dificuldade
## em interpretar o gráfico
...
qplot(rolls, binwidth = 1)
  ...
  # Mas, e agora? Como gerar um dado "injusto"? Vamos olhar para a ajuda da função sample e ver o que ela nos diz
?sample()
??sample()
  # Apesar do pacote nos indicar somente que existe o argumento "prob", não nos indica como funciona. Nesse caso, deve-se saber que
  ## são indicadas as probabilidades de cada um dos valores seguindo a ordem do vetor construído
...
qplot(rolls2, binwidth = 1)
  # Caso utilizássemos uma amostra menor, digamos 10 jogadas, não teríamos a ideia do dado viciado. Isso só é possível em decorrência
  ## da grande amostra utilizada.
